// Source audit: a screen on UiListActivity builds its rows and lets the base
// draw the chrome. The moment one of them paints its own header, hints or rows,
...
#ifndef LIST_SCREEN_SOURCES
#error "LIST_SCREEN_SOURCES must be defined by the build system"
#endif
...
std::vector<std::string> sourcePaths() {
  ...
  std::stringstream all(LIST_SCREEN_SOURCES);
  std::string path;
...
TEST(ListScreenAudit, EveryListScreenBuildsItsOwnScreen) {
  for (const std::string& path : sourcePaths()) {
    std::ifstream file(path);
    ASSERT_TRUE(file.is_open()) << "cannot open " << path;
    std::stringstream body;
    body << file.rdbuf();
    EXPECT_NE(body.str().find("buildScreen"), std::string::npos)
        << path << " is listed as a list screen but never builds a screen.";
  }
}
