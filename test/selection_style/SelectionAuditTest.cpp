...
TEST(SelectionAudit, EverySelectionSurfaceCallsTheSharedPainter) {
  ...
  // rendered cover and a restored one, plus the two hand-rolled lists left outside
  // BaseTheme: the OPDS browser and the XTC chapter list. The nearby peer list was
  // one of them until it moved onto UiStatusActivity, where FreeInkUI marks the
  // selection.
  EXPECT_GE(calls, 10);
}
