...
void ParsedText::extractLine(const size_t breakIndex, const int pageWidth, const std::vector<uint16_t>& wordWidths,
  ...
  // Guide dots (LTR only; never with BiDi reordering). A dot sits in the gap before
  ...
  if (willReorder) {
    ...
    JustifySpacing reorderedJustifySpacing(
        effectiveAlignment == CssTextAlign::Justify && !isLastLine ? reorderedSpare : 0, reorderedGapCount);
    const int contentWidth = reorderedWordWidthSum + reorderedNaturalGaps + reorderedJustifySpacing.totalExtra;
    ...
    for (size_t wordIdx = 0; wordIdx < reorderedWidthsScratch.size(); wordIdx++) {
      ...
      if (nextIsContinuation) {
        ...
        if (wordIdx > 0 && reorderedWordsScratch[wordIdx] == " " && reorderedContinuesScratch[wordIdx] &&
          ...
          advance += reorderedJustifySpacing.nextExtra();
        }
        xpos += advance;
      ...
      for (size_t wordIdx = 0; wordIdx < lineWordCount; wordIdx++) {
        ...
        if (nextIsContinuation) {
          ...
          if (wordIdx > 0 && lineWords[wordIdx] == " " && continuesVec[lastBreakAt + wordIdx] &&
              effectiveAlignment == CssTextAlign::Justify && !isLastLine) {
            advance += justifySpacing.nextExtra();
          }
          xpos -= advance;
        } else {
          int gap = 0;
  ...
  if (lineHasGuideDot) outGuideDotXOffset.assign(lineWordCount, 0);
  ...
  for (size_t i = 0; i < lineWordCount; i++) {
    ...
    // A guide dot precedes this word: centre it in the gap between the previous word and this
    // one, using the already-justified lineXPos so it stays centred after any stretch. The
    // offset is stored on the previous word, which is where TextBlock::render() reads it.
    if (lineHasGuideDot && i > 0 && guideDotBeforeLine(i)) {
      const int prevX = static_cast<int>(lineXPos[i - 1]);
      const int prevWidth =
          measureFocusWordWidth(renderer, fontId, lineWords[i - 1], lineWordStyles[i - 1], outBoundaries[i - 1]);
      ...
      outGuideDotXOffset[i - 1] = static_cast<uint16_t>(dotDelta > 0 ? dotDelta : 0);
    }
  }

  auto block = std::make_shared<TextBlock>(lineWords, lineXPos, lineWordStyles, outBoundaries, outSuffixX,
                                           outGuideDotXOffset, blockStyle, std::move(lineRubyTexts));
  if (!block->valid()) {
    LOG_ERR("PTX", "Dropping line: TextBlock arena allocation failed");
    return;
  }
