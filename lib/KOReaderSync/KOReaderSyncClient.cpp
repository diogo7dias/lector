...
// KOSync's TLS-1.3 servers cannot be reached through the precompiled system
...
// floor and check both total free heap and the largest contiguous block so a
// fragmented heap does not fall through into a failed TLS allocation. (#2475)
...
KOReaderSyncClient::Error KOReaderSyncClient::authenticate() {
  ...
  http.setInsecure();
  if (!http.begin(url)) {
    LOG_ERR("KOSync", "begin failed for %s", url.c_str());
