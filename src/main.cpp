...
void enterDeepSleep(bool fromTimeout = false) {
  ...
  // Reader-resume wakes no longer restore the page frame: per Diogo, a page
  // that is visible but not yet interactive reads as "stuck", so the wake
  // keeps the boot screen (wallpaper + unlock banners) until the book engine
  // is actually ready, then paints the page with the buttons live. No frame
  // to save here; quick-resume sleeps keep their post-sleep-screen save
  // (their sleep screen IS the retained frame).
...
void setup() {
  ...
  switch (resume) {
    ...
    case BootResume::Splash:
      // Reader-resume wakes keep the boot screen (wallpaper + unlock banners)
      // until the reader's first real paint: the page appears exactly when the
      // buttons are live, instead of appearing early and playing dead while
      // the chapter loads (Diogo: a visible-but-unresponsive page reads as
      // "stuck"; a loading face that flips straight into a working page does
      // not).
