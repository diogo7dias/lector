    ...
    switch (s.type) {
      ...
      case SettingType::ENUM: {
        ...
        if (s.valuePtr) {
          ...
          doc["value"] = static_cast<int>(s.valueGetter(s.dynCtx));
        }
      ...
      case SettingType::STRING: {
        doc["type"] = "string";
        if (s.stringGetter) {
          doc["value"] = s.stringGetter(s.dynCtx);
      ...
      case SettingType::ENUM: {
        ...
        if (val >= 0 && val < maxVal) {
          if (s.valuePtr) {
            ...
            s.valueSetter(s.dynCtx, static_cast<uint8_t>(val));
          }
          applied++;
        }
        break;
