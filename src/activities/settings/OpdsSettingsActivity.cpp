...
void OpdsSettingsActivity::handleSelection() {
  ...
  if (selectedIndex == 0) {
    ...
    // Keep server filename - toggle On/Off
...
void OpdsSettingsActivity::render(RenderLock&&) {
  ...
  GUI.drawList(
      ...
      [this, &fieldNames](int index) -> std::string {
      ...
      [this](int index) -> std::string {
        if (index == 0) {
          ...
          return editServer.keepFilename ? std::string(tr(STR_STATE_ON)) : std::string(tr(STR_STATE_OFF));
        }
        return std::string("");
      },
      true);
