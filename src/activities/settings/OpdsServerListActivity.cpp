...
namespace {
// Normalizes a user-typed folder: trims spaces, "" => SD root, otherwise a single
// leading '/' and no trailing '/'. Cold path (runs once per edit). Upstream #2571.
...
int OpdsServerListActivity::getItemCount() const {
  ...
  // and "Filename format"; picker mode shows only real servers.
  if (!pickerMode) {
    count += 3;
  }
  return count;
