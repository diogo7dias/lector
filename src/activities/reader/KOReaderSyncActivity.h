...
class KOReaderSyncActivity final : public Activity {
 ...
 private:
  enum State {
    ...
    UPLOADING,
    UPLOAD_COMPLETE,
    SYNC_COMPLETE,  // smart sync resolved to "already synced" (no change needed)
    NO_REMOTE_PROGRESS,
    SYNC_FAILED,
    NO_CREDENTIALS
  };
  ...
  // Timed return for successful terminal states (smart sync + uploads), so a
  // resolved sync drops back to the reader on its own after a short confirmation.
