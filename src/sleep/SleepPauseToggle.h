...
// favorite suffix.
// The rotation folder is /sleep or /.sleep (SleepFolderPolicy picks one); a paused
// wallpaper is restored to whichever the index currently reads.
constexpr const char* kSleepPauseDir = "/sleep pause";

struct SleepPauseToggleResult {
  bool ok = false;       // the move succeeded
  bool toPause = false;  // true = moved into "/sleep pause", false = moved back to rotation
  std::string newPath;   // the file's location after the move (== input path on failure)
};

// If `path` sits directly under /sleep, /.sleep or "/sleep pause", move it to the other of
...
// True when `path` is directly inside /sleep, /.sleep or "/sleep pause" (i.e. togglable).
bool isUnderSleepDirs(const std::string& path);

// True when `path` is inside "/sleep pause": the toggle would restore it.
bool isPaused(const std::string& path);
